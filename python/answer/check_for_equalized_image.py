...
imagePath = "/home/opencv-mds/OpenCV_in_Ubuntu/Data/Lane_Detection_Images/solidYellowCurve.jpg"
...
while True:
    ...
    hsv = convertColor(image, cv2.COLOR_BGR2HSV)
    h, s, v = splitImage(hsv)
    s = histogramEqualize(s)
    v = histogramEqualize(v)
    hsv = mergeImage(h, s, v)
    rgb = convertColor(hsv, cv2.COLOR_HSV2RGB)
    hls = convertColor(rgb, cv2.COLOR_RGB2HLS)
    img = splitColor(hls, lower, upper)
