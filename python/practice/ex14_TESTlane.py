...
def imageProcessing2(image):
    ...
    cv2.imshow("test 1",result)
    

    lower = np.array([180, 180, 180])
    upper = np.array([255, 255, 255])
    whiteBGR = rangeColor(result, lower, upper)
    cv2.namedWindow("test 2",cv2.WINDOW_GUI_EXPANDED)
    cv2.imshow("test 2",whiteBGR)

    #edge = cannyEdge(whiteBGR, 100, 200);
    #cv2.namedWindow("test 3",cv2.WINDOW_GUI_EXPANDED)
    #cv2.imshow("test 3", edge)

    lines = houghLinesP(whiteBGR, 1, np.pi/180, 40)
    empty = np.zeros((h,w), np.uint8)
    result = drawHoughLinesP(empty, lines)
    cv2.namedWindow("test 3",cv2.WINDOW_GUI_EXPANDED)
    cv2.imshow("test 3", result)

    result = lineFitting(image, lines, (0,0,255),5,5.*np.pi/180.)
    cv2.namedWindow("test 4",cv2.WINDOW_GUI_EXPANDED)
    cv2.imshow("test 4", result)
