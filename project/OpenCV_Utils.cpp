...
void lineFitting(Mat &image, Mat &result, vector<Vec4i> &lines, Scalar color, int thickness, float slope_threshold)
{
    ...
    int min_y = int(height * 0.5);
    int max_y = int(height * 0.8);
    ...
    /*
    vector<Point> pts;
    pts.push_back(Point(min_x_left+5, min_y));
    pts.push_back(Point(min_x_right-5, min_y));
    pts.push_back(Point(max_x_right-5, max_y));
    pts.push_back(Point(max_x_left+5, max_y));
    vector<vector<Point> > fillContAll;
    fillContAll.push_back(pts);
    fillPoly(result, fillContAll, Scalar(0, 255, 0));*/

    
    lower_x = (min_x_left+min_x_right)/2.;    
    upper_x = (max_x_left+max_x_right)/2.;
    line(result, Point(lower_x, min_y), Point(upper_x,max_y), Scalar(255, 0, 0), 5);
